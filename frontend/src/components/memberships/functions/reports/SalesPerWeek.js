...
const SalesPerWeek_style = ({anio, semana, tracks}) => {
  const style = {
    color: '#FFFFFF'
  }
...
const SalesPerWeek = () => {
  ...
  const [selledTrack,setSelledTrack]= useState([])
  ...
  const [showError, setShowError] = useState('')
    ...
    async function getSalesPerWeek() {
      var temporary = []
      const json = await fetch('http://localhost:3001/SalesPerWeek')
    ...
    <div>
        ...
        <center>
          ...
          <button onClick={ getSalesPerWeek } className='botones'>
            <font color ='#FFFFFF'>
              <b>
                Submit
              </b>
        ...
        <center>
          <div class='column'>
            ...
            <div>{selledTrack.map((result) => {
                    return <SalesPerWeek_style anio={result.anio} 
